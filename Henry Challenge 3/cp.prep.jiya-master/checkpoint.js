...
function stringMasLarga(str) {
  ...
  // Tu código aca:
  var texto = 'Ayer fui a pasear a una plaza';
  var string = texto.split(' ');
  var resultado = "";
  
  for (var i = 0; i < string.length; i++) {
    if (string[i].length > resultado.length) {
      resultado = string[i];
    };
  };
    return resultado;
...
function actividadesEnComun(persona1, persona2) {
  ...
  for (var i = 0; i < persona1.length; i++)
                                      ...
                                      {
for (var j = 0; j < persona2.length; i++)                                                                            
}                               {
if (persona1[i] === persona2[j])
iguales++;
};
var arrayEnComun[iguales];
return arrayEnComun;
